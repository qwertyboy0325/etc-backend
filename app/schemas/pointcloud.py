...
class PointCloudFileResponse(PointCloudFileBase):
    ...
    upload_completed_at: Optional[datetime] = Field(None, description="Upload completion time")
...
class FileUploadResponse(BaseModel):
    ...
    upload_url: Optional[str] = Field(None, description="Temporary upload URL if needed")
    point_count: Optional[int] = Field(None, description="Number of points")
    bounding_box: Optional[BoundingBox] = Field(None, description="3D bounding box")
...
class PointCloudAnalysis(BaseModel):
    ...
    # Statistics
    density: Optional[float] = Field(None, description="Point density (points per unit volume)")
    coverage_area: Optional[float] = Field(None, description="Coverage area (square units)")
    
    # Quality assessment
    estimated_noise_level: Optional[float] = Field(None, description="Estimated noise level")
    outlier_percentage: Optional[float] = Field(None, description="Percentage of outlier points")
    uniformity_score: Optional[float] = Field(None, description="Point distribution uniformity (0-1)")


class PointCloudPreview(BaseModel):
    """Schema for point cloud preview data."""
    
    sample_points: List[List[float]] = Field(..., description="Sample points for preview")
